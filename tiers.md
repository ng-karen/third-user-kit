...
- **Passes the two-minute test** — no invented props, and a correct "when not to use" ([two-minute-test.md](two-minute-test.md))
...
An agent sees a flat list of files, all equally valid, and reaches for whichever name sounds closest. The tier — surfaced in the [index](component-index.template.md) — tells it which components are load-bearing defaults and which solve one flow's niche problem. Same map a new engineer needs. Same file.
